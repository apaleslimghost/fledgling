...
describe('TagTree', () => {
	...
	it('should bubble up descendent notes', (t: TestContext) => {
		...
		t.assert.deepStrictEqual(tree.notes, [notes[0], notes[1], notes[2], notes[3]])

		t.assert.deepStrictEqual(tree.get(['a']).notes, [notes[0], notes[1]])

		t.assert.deepStrictEqual(tree.get(['f', 'g']).notes, [notes[3]])
	})
})
